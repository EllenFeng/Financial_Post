...
public class BankNews extends AppCompatActivity implements Runnable{
...
//                String link2 = String.valueOf(link.getText());
                ...
                intent.setClass(BankNews.this, NewsContent.class);
    ...
    public void run() {//线程
        ...
        try {
            Document doc = Jsoup.connect("http://finance.sina.com.cn/money/bank/").get();
            Elements titles = doc.select(".blk05_cont a");
            Elements titles1 = doc.select(".blk06 a");
            Elements titles2 = doc.select(".blk07 a");
            Elements titles3 = doc.select(".blk09 a");
            ...
            for (int i = 0; i < titles3.size(); i+=1) {
                Element newstitle = titles3.get(i);
                String titleStr = newstitle.text();
                String linkStr = newstitle.attr("href");
                if(titleStr.length()<1)
                    continue;
